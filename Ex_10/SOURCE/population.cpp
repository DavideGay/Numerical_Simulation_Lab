...
void Population :: initialize(int setrank){
    ...
    if (Primes.is_open()){
        for(int i=0; i<setrank; i++){   // allows running different number generators for each process
            Primes >> p1 >> p2 ;
        }
    ...
    if(cities_type==0){
        ...
        if(counter<3){
            ...
            N_cities=counter;   // overrides value of N_cities obtained from 'input.dat'
        }
    ...
    for(int i=0; i<population_size; i++){
        pop[i].initialize(N_cities);   // creates population whose paths are all "1, 2, ..., N_cities" 
    }
...
vector<int> Population::get_single_path(int index){
    return pop[index].get_steps();
}

void Population::set_single_path(vector<int>& first, int index){
    pop[index].set_steps(first);
}
...
void Population::mutation_pair(double probability){
    for(int i=0; i<pop_size; i++){
        if(rnd.Rannyu()<probability){
            int a=int(rnd.Rannyu(1, N_cities));
            int b=int(rnd.Rannyu(1, N_cities));
            while(a==b){
                b=int(rnd.Rannyu(1, N_cities));
            }
            pop[i].path_swap(a, b);
        }
    }
...
void Population::mutation_inversion(double probability){
    for(int i=0; i<pop_size; i++){
        if(rnd.Rannyu()<probability){
            int begin=int(rnd.Rannyu(0,N_cities-1));
            int end=N_cities;
            if(N_cities-begin>2){
                end=int(rnd.Rannyu(begin+2,N_cities+1));
            }
...
void Population::mutation_shift(double probability){
    for(int i=0; i<pop_size; i++){
        if(rnd.Rannyu()<probability){
            int end=int(rnd.Rannyu(0,N_cities));
            int begin=int(rnd.Rannyu(1,end));
            int step=int(rnd.Rannyu(1, N_cities-end));
...
void Population::single_crossover(int index1, int index2, int cut_index){
    vector<int> tempv1;
    tempv1.resize(N_cities-cut_index);
    vector<int> tempv2;
    tempv2.resize(N_cities-cut_index);
    for(int i=cut_index; i<N_cities; i++){
...
void Population::crossover(double exponent){
    ...
    Population newgen;
    newgen.quick_initialize(pop_size, N_cities, rnd);
    newgen.shuffle_all();
    ...
    for(int i=0; i<pop_size-1; i=i+2){  // if pop_size is an odd number, the last individual is a randomly generated one
        ...
        newgen.single_crossover(i, i+1, int(rnd.Rannyu(1,N_cities)));
...
void Population::print_single_path(ofstream& outfile, int index){
    pop[index].print_path(outfile, cities);
}
