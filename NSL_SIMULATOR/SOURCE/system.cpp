...
void System :: move(int i){ // Propose a MC move for particle i
  if(_sim_type == 3){ //Gibbs sampler for Ising
    double delta_E = 2. * (_J * (_particle(this->pbc(i-1)).getspin() + _particle(this->pbc(i+1)).getspin()) + _H);
		double prob_i_up = 1. / (1. + exp (-1.*_beta * delta_E)); // conditional distribution
...
void System :: initialize(){ // Initialize the System object according to the content of the input files in the ../INPUT/ directory
  ...
  string property;
  double delta;
  _tails=0; // initialize tails as false
  _equilibrated=0;
  while ( !input.eof() ){
    input >> property;
  ...
  if (input.is_open()){
    while ( !input.eof() ){
      ...
      if( property == "POTENTIAL_ENERGY" ){
        ...
        index_property++;
        _vtail=0;
        if(_sim_type<2 && _tails) _vtail = 8.*M_PI*_rho /(9.* pow(_r_cut, 9.)) - 8.*M_PI*_rho /(3.* pow(_r_cut, 3.));
        ...
        index_property++;
        _ptail=0;
        if(_sim_type<2 && _tails) _ptail = 32. * M_PI * _rho * ( (1./(9.*pow(_r_cut,9))) - (1./(6.*pow(_r_cut,3))) );
...
void System :: measure(){ // Measure properties
  ...
  if (_measure_penergy or _measure_pressure or _measure_gofr) {
    for (int i=0; i<_npart-1; i++){
      for (int j=i+1; j<_npart; j++){
        ...
        distance(2) = this->pbc( _particle(i).getposition(2,true) - _particle(j).getposition(2,true), 2);
        dr = sqrt( dot(distance,distance) );
        if(dr < _halfside.min() and _measure_gofr){  // GOFR
					_measurement(_index_gofr + int(floor(dr/double(_bin_size)))) += 2 ; // updates distance values "histogram"
        }
        if(dr < _r_cut){   // potential cutoff!
          if(_measure_penergy)  penergy_temp += 1.0/pow(dr,12) - 1.0/pow(dr,6); // POTENTIAL ENERGY
          if (_measure_pressure) virial += 1.0/pow(dr,12) - 0.5 /pow(dr,6); // to be used for computing pressure
        }
      }
    }
  }
  // POTENTIAL ENERGY //////////////////////////////////////////////////////////
  if (_measure_penergy){
    penergy_temp = 4.0 * penergy_temp / double(_npart) + _vtail;
  ...
  if (_measure_tenergy){
    if (_sim_type < 2) _measurement(_index_tenergy) = kenergy_temp + penergy_temp;
      ...
      tenergy_temp /= double(_npart);   // per particle !!!
  ...
  if (_measure_pressure){
    virial = 48./(3. *_volume) * virial;
    _measurement(_index_pressure) = virial + ((2.0/3.0)*kenergy_temp) *_rho + _ptail;
    // 2/3 kenergy is the temperature calculated at present time
  }
  // TOTAL SPIN ////////////////////////////////////////////////////////////////
  if (_sim_type > 1){
		for(int i = 0; i<_npart; i++){
			tot_spin += double(_particle(i).getspin());
    }
  }
  // MAGNETIZATION /////////////////////////////////////////////////////////////
  if (_measure_magnet){
    if(_sim_type<2) _measurement(_index_magnet) = 0;
    else{
      _measurement(_index_magnet) = tot_spin/_npart;
    }
  }
  // SPECIFIC HEAT /////////////////////////////////////////////////////////////
  if (_measure_cv){
    if(_sim_type<2) _measurement(_index_cv) = 0;
    else{
      _measurement(_index_cv) = pow(tenergy_temp*_npart, 2);   // tenergy^2; calulation is completed in averages
    }
  }
  // SUSCEPTIBILITY ////////////////////////////////////////////////////////////
  if (_measure_chi){
    if(_sim_type<2) _measurement(_index_chi) = 0;
    else{
      _measurement(_index_chi) = _beta* pow(tot_spin,2);
    }
  }
...
void System :: measure_outputs(int blk){
  if(_sim_type>1 and _equilibrated){
    ofstream coutf;

    if (_measure_tenergy){
      coutf.open("../OUTPUT/temp_tenergy.dat",ios::app);
      coutf << setw(12) << _temp
            << setw(12) << _global_av(_index_tenergy)/double(blk)
            << setw(12) << this->error(_global_av(_index_tenergy), _global_av2(_index_tenergy), blk) << endl;
      coutf.close();
    }

    if(_measure_magnet){
      coutf.open("../OUTPUT/temp_magnetization.dat",ios::app);
      coutf << setw(12) << _temp
            << setw(12) << _global_av(_index_magnet)/double(blk)
            << setw(12) << this->error(_global_av(_index_magnet), _global_av2(_index_magnet), blk) << endl;
      coutf.close();
    }

    if(_measure_cv){
      coutf.open("../OUTPUT/temp_specific_heat.dat",ios::app);
      coutf << setw(12) << _temp
    ...
    if(_measure_chi){
      coutf.open("../OUTPUT/temp_susceptibility.dat",ios::app);
      coutf << setw(12) << _temp
            << setw(12) << _global_av(_index_chi)/double(blk)
            << setw(12) << this->error(_global_av(_index_chi), _global_av2(_index_chi), blk) << endl;
      coutf.close();
    }
  }
...
void System :: averages(int blk){
  ...
  if (_measure_cv){
    if(_sim_type>1){
      _average(_index_cv) = (_average(_index_cv) - pow((_average(_index_tenergy)*_npart),2) ) * _beta*_beta/_npart;
    }
  }
  ...
  // GOFR //////////////////////////////////////////////////////////////////////
  if (_measure_gofr && blk==_nblocks && _sim_type<2){
